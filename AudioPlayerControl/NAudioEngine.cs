...
namespace Sample_NAudio
{
    ...
    internal class NAudioEngine :  IWaveformPlayer, ISoundPlayer, INotifyPropertyChanged, IDisposable
    {
        ...
        /// <summary>
        /// Used for update waveFormTimeLine Element
        /// </summary>
        private readonly BackgroundWorker timeLineWorker = new BackgroundWorker();
        ...
        private NAudioEngine()
        {
            ...
            this.timeLineWorker.DoWork += timeLineWorker_DoWork;    
            this.timeLineWorker.RunWorkerCompleted += timeLineWorker_RunWorkerCompleted;
        }
        ...
        private void timeLineWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            processActiveStream();
            ChannelLength = ActiveStream.TotalTime.TotalSeconds;

            if ((double)e.Result != ActiveStream.TotalTime.TotalSeconds)
                timeLineWorker.RunWorkerAsync();
        }

        private void timeLineWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            e.Result = ActiveStream.TotalTime.TotalSeconds;

            Thread.Sleep(100);
        }

        public void SetData(ref Stream channelStream, int rate, int bits, int channel)
        {
            WaveFormat waveFormat = new WaveFormat(rate, bits, channel);
            Stop();
            StopAndCloseStream();
            ...
            try
            {
                WaveOut outDevice = new WaveOut { DesiredLatency = 500 }; // (WaveCallbackInfo.FunctionCallback());
                waveOutDevice = outDevice;
                ActiveStream = new RawSourceWaveStream(channelStream, waveFormat);

                _streamToPlay = new WaveChannel32((RawSourceWaveStream)ActiveStream);
                _streamToPlay.Sample += ForwardStreamSample;
                waveOutDevice.Init(_streamToPlay);
                ChannelLength = _streamToPlay.TotalTime.TotalSeconds;
